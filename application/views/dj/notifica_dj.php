...
<script>
    ...
    function cargarNotificaciones(filtro) {
        fetch(`<?= base_url() . 'dj/notificaciones_ajax' ?>/${filtro}`)
            .then(res => res.json())
            .then(data => {
                ...
                fetch('<?= base_url() ?>dj/contadores_notificaciones')
                    .then(res => res.json())
                    .then(contadores => {
    ...
    document.getElementById('lista-notificaciones').addEventListener('click', function(e) {
        ...
        if (!target.classList.contains('papelera') && !target.closest('.borrar-notificacion')) {
            ...
            fetch(`<?= base_url() ?>dj/marcar_leida`, {
                    method: 'POST',
                    headers: {
        ...
        if (target.classList.contains('papelera') || target.closest('.borrar-notificacion')) {
            ...
            fetch(`<?= base_url() . 'dj/borrar_notificacion' ?>`, {
                    method: 'POST',
                    headers: {
                        'Content-Type': 'application/x-www-form-urlencoded'
                    },
    ...
    document.getElementById('btn-borrar-todo').addEventListener('click', () => {
        ...
        fetch(`<?= base_url() ?>dj/borrar_todas_notificaciones`, {
                method: 'POST'
            })
            .then(res => res.json())
            .then(data => {
